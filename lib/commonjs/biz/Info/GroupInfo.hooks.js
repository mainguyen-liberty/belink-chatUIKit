...
var _useGroupInfoActions = require("../hooks/useGroupInfoActions");
...
function useGroupInfo(props, ref) {
  const {
    groupId,
    ownerId,
  ...
  const init = React.useCallback(() => {
    ...
    im.getConversation({
      convId: groupId,
      convType: _reactNativeChatSdk.ChatConversationType.GroupChat,
  ...
  } = (0, _useGroupInfoActions.useGroupInfoActions)({
    menuRef,
    alertRef,
  ...
  const onCopyId = () => {
    ...
    _services.Services.cbs.setString(groupId);
  ...
  React.useEffect(() => {
    const listener = {
      ...
      onGroupEvent: (event, target, _usernames) => {
        if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.GROUP_ASSIGN_OWNER) {
          if (target === groupId) {
            if (im.userId === target) {
