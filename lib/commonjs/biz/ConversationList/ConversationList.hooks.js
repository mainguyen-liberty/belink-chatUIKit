...
exports.sortConversations = void 0;
...
var _useConversationListMoreActions = require("../hooks/useConversationListMoreActions");
var _useConversationLongPressActions = require("../hooks/useConversationLongPressActions");
var _List = require("../List");
var _ConversationList = require("./ConversationList.item");
...
function useConversationList(props) {
  const {
    onClickedItem,
    onLongPressedItem,
    testMode,
  ...
  const ListItemRenderRef = React.useRef(propsListItemRender ?? _ConversationList.ConversationListItemMemo);
  ...
  } = (0, _useConversationListMoreActions.useConversationListMoreActions)({
    alertRef,
    menuRef,
    onClickedNewContact,
    onClickedNewConversation,
        ...
        return {
          ...
          data: {
            ...
            lastMessage: i % 4 === 0 ? undefined : {
              ...
              body: {
                type: _reactNativeChatSdk.ChatMessageType.TXT,
                content: 'hello'
              }
            }
          },
            ...
            if (isOk && list) {
              ...
              if (list) {
                for (const conv of list) {
                  if (conv.convId === _chat.gNewRequestConversationId) {
                    continue;
                  }
                  dataRef.current.push({
                    id: conv.convId,
                ...
                if (isShowAfterLoaded === true) {
                  dataRef.current = dataRef.current.filter(item => item.data.convId !== _chat.gNewRequestConversationId);
  ...
  } = (0, _useConversationLongPressActions.useConversationLongPressActions)({
    menuRef,
    alertRef,
  ...
  const listener = React.useMemo(() => {
    return {
      ...
      onConversationEvent: (event, convId, convType) => {
        if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONVERSATION_DELETED) {
          if (convId) {
            init({});
          }
        } else if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONVERSATION_PINNED) {
          if (convId && convType !== undefined) {
            onPin(convId, convType);
          }
        } else if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONVERSATION_UNPINNED) {
          if (convId && convType !== undefined) {
  ...
  if (propsRef && propsRef.current) {
    ...
    propsRef.current.refreshList = () => {
      im.sendUIEvent(_chat.UIListenerType.Conversation, 'onRequestRefreshEvent');
    };
    propsRef.current.reloadList = () => {
      im.sendUIEvent(_chat.UIListenerType.Conversation, 'onRequestReloadEvent');
    };
...
const sortConversations = (prevProps, nextProps) => {
