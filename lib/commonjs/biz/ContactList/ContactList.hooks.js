...
var _useContactListMoreActions = require("../hooks/useContactListMoreActions");
var _List = require("../List");
var _const = require("./const");
var _ContactList = require("./ContactList.item");
...
function useContactList(props) {
  ...
  const sectionListProps = (0, _List.useSectionList)({
  ...
  const ListItemRenderRef = React.useRef(propsListItemRender ?? _ContactList.ContactListItemMemo);
  const ListItemHeaderRenderRef = React.useRef(propsListItemHeaderRender ?? _ContactList.ContactListItemHeaderMemo);
  const {
    onShowContactListMoreActions
  } = (0, _useContactListMoreActions.useContactListMoreActions)({
    menuRef,
    alertRef
  });
  ...
  const getFirst = React.useCallback(str => {
    ...
    if (str && str.length > 0) {
      ...
      ret = first;
      if ((0, _utils.containsChinese)(first)) {
        var _getPinyinFirsLetter$;
        ret = (_getPinyinFirsLetter$ = (0, _utils.getPinyinFirsLetter)(first).at(0)) === null || _getPinyinFirsLetter$ === void 0 ? void 0 : _getPinyinFirsLetter$.toLocaleUpperCase();
      }
    }
    return ret;
  }, []);
  ...
  const refreshToUI = React.useCallback(list => {
    ...
    uniqueList.forEach(item => {
      ...
      const indexTitle = first ? _const.g_index_alphabet_range.includes(first) ? first : '#' : '#';
  ...
  const contactItems = React.useCallback(_ref => {
    ...
    const items = [/*#__PURE__*/React.createElement(_ContactList.ContactItem, {
      name: tr('_uikit_contact_new_request'),
      count: /*#__PURE__*/React.createElement(_Badges.Badges, {
        count: requestCount
      }),
      hasArrow: true,
      onClicked: onClickedNewRequest
    }), /*#__PURE__*/React.createElement(_ContactList.ContactItem, {
      name: tr('_uikit_contact_group_list'),
      count: /*#__PURE__*/React.createElement(_Text.Text, {
        paletteType: 'label',
        textType: 'medium'
      }, null),
      hasArrow: true,
  ...
  (0, _chat.useChatListener)(React.useMemo(() => {
    return {
      ...
      onConversationEvent: (event, convId, _convType) => {
        if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONTACT_REMOVE) {
          if (convId) {
      ...
      onContactEvent: event => {
        if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONTACT_ACCEPT || event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONTACT_REMOVE) {
          init({});
        }
      }
    };
  ...
  const ListHeaderComponent = React.useCallback(() => {
    ...
    return /*#__PURE__*/React.createElement(_reactNative.View, null, ret);
