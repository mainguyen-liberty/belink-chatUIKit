"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.ContactItem = ContactItem;
exports.ContactListItem = ContactListItem;
exports.ContactListItemHeader = ContactListItemHeader;
exports.ContactListItemMemo = exports.ContactListItemHeaderMemo = void 0;
var React = _interopRequireWildcard(require("react"));
var _reactNative = require("react-native");
var _hook = require("../../hook");
var _theme = require("../../theme");
var _Button = require("../../ui/Button");
var _Image = require("../../ui/Image");
var _Text = require("../../ui/Text");
var _Avatar = require("../Avatar");
var _ListItem = require("../ListItem");
function _getRequireWildcardCache(nodeInterop) { if (typeof WeakMap !== "function") return null; var cacheBabelInterop = new WeakMap(); var cacheNodeInterop = new WeakMap(); return (_getRequireWildcardCache = function (nodeInterop) { return nodeInterop ? cacheNodeInterop : cacheBabelInterop; })(nodeInterop); }
function _interopRequireWildcard(obj, nodeInterop) { if (!nodeInterop && obj && obj.__esModule) { return obj; } if (obj === null || typeof obj !== "object" && typeof obj !== "function") { return { default: obj }; } var cache = _getRequireWildcardCache(nodeInterop); if (cache && cache.has(obj)) { return cache.get(obj); } var newObj = {}; var hasPropertyDescriptor = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var key in obj) { if (key !== "default" && Object.prototype.hasOwnProperty.call(obj, key)) { var desc = hasPropertyDescriptor ? Object.getOwnPropertyDescriptor(obj, key) : null; if (desc && (desc.get || desc.set)) { Object.defineProperty(newObj, key, desc); } else { newObj[key] = obj[key]; } } } newObj.default = obj; if (cache) { cache.set(obj, newObj); } return newObj; }
/**
 * Contact list item component.
 */
function ContactListItem(props) {
  const {
    section,
    onClicked,
    onCheckClicked,
    onLongPressed
  } = props;
  const {
    checked,
    disable
  } = section;
  const {
    colors
  } = (0, _theme.usePaletteContext)();
  const {
    getColor
  } = (0, _hook.useColors)({
    bg: {
      light: colors.neutral[98],
      dark: colors.neutral[1]
    },
    t1: {
      light: colors.neutral[1],
      dark: colors.neutral[98]
    },
    t2: {
      light: colors.neutral[5],
      dark: colors.neutral[6]
    },
    divider: {
      light: colors.neutral[9],
      dark: colors.neutral[2]
    },
    disable: {
      light: colors.neutral[7],
      dark: colors.neutral[4]
    },
    enable: {
      light: colors.primary[5],
      dark: colors.primary[6]
    }
  });
  return /*#__PURE__*/React.createElement(_reactNative.View, {
    style: {
      backgroundColor: getColor('bg')
    }
  }, /*#__PURE__*/React.createElement(_reactNative.Pressable, {
    style: {
      width: '100%',
      height: 59.5,
      flexDirection: 'row',
      alignItems: 'center',
      paddingHorizontal: 16
    },
    onPress: () => {
      if (checked !== undefined) {
        if (disable !== true) {
          onCheckClicked === null || onCheckClicked === void 0 ? void 0 : onCheckClicked(section);
        }
      } else {
        onClicked === null || onClicked === void 0 ? void 0 : onClicked(section);
      }
    },
    onLongPress: () => {
      onLongPressed === null || onLongPressed === void 0 ? void 0 : onLongPressed(section);
    }
  }, checked !== undefined ? /*#__PURE__*/React.createElement(_reactNative.View, {
    style: {
      marginRight: 12
    }
  }, /*#__PURE__*/React.createElement(_Button.IconButton, {
    iconName: checked !== false ? 'checked_rectangle' : 'unchecked_rectangle',
    style: {
      height: 28,
      width: 28,
      tintColor: getColor(checked !== false && disable !== true ? 'enable' : 'disable')
    },
    onPress: () => {
      if (disable !== true) {
        onCheckClicked === null || onCheckClicked === void 0 ? void 0 : onCheckClicked(section);
      }
    }
  })) : null, /*#__PURE__*/React.createElement(_Avatar.Avatar, {
    url: section.userAvatar,
    size: 40
  }), /*#__PURE__*/React.createElement(_reactNative.View, {
    style: {
      flexGrow: 1,
      paddingLeft: 12,
      maxWidth: checked !== undefined ? '70%' : '80%'
    }
  }, /*#__PURE__*/React.createElement(_Text.SingleLineText, {
    paletteType: 'title',
    textType: 'medium',
    style: {
      color: getColor('t1')
    }
  }, section.userName ?? section.userId))), /*#__PURE__*/React.createElement(_reactNative.View, {
    style: {
      width: '100%',
      borderBottomWidth: 0.5,
      borderBottomColor: getColor('divider'),
      marginLeft: 68
    }
  }));
}
const ContactListItemMemo = /*#__PURE__*/React.memo(ContactListItem);

/**
 * Contact list item header component.
 */
exports.ContactListItemMemo = ContactListItemMemo;
function ContactListItemHeader(props) {
  const {
    indexTitle
  } = props;
  const {
    colors
  } = (0, _theme.usePaletteContext)();
  const {
    getColor
  } = (0, _hook.useColors)({
    bg: {
      light: colors.neutral[98],
      dark: colors.neutral[1]
    },
    t1: {
      light: colors.neutral[1],
      dark: colors.neutral[98]
    },
    t2: {
      light: colors.neutral[5],
      dark: colors.neutral[6]
    },
    divider: {
      light: colors.neutral[9],
      dark: colors.neutral[2]
    }
  });
  return /*#__PURE__*/React.createElement(_reactNative.View, {
    style: [{
      backgroundColor: getColor('bg')
    }]
  }, /*#__PURE__*/React.createElement(_reactNative.View, {
    style: [{
      height: 32,
      justifyContent: 'center',
      paddingLeft: 16
    }]
  }, /*#__PURE__*/React.createElement(_Text.SingleLineText, {
    paletteType: 'title',
    textType: 'small',
    style: {
      color: getColor('t2')
    }
  }, indexTitle)));
}
const ContactListItemHeaderMemo = /*#__PURE__*/React.memo(ContactListItemHeader);

/**
 * Contact list item header component.
 */
exports.ContactListItemHeaderMemo = ContactListItemHeaderMemo;
function ContactItem(props) {
  const {
    icon,
    name,
    count,
    hasArrow,
    onClicked
  } = props;
  const {
    colors
  } = (0, _theme.usePaletteContext)();
  const {
    getColor
  } = (0, _hook.useColors)({
    bg: {
      light: colors.neutral[1],
      dark: colors.neutral[98]
    },
    icon: {
      light: colors.neutral[6],
      dark: colors.neutral[5]
    }
  });
  return /*#__PURE__*/React.createElement(_ListItem.ListItem, {
    LeftName: /*#__PURE__*/React.createElement(_reactNative.View, {
      style: {
        flexDirection: 'row',
        alignItems: 'center'
      }
    }, icon ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(_Image.Icon, {
      name: icon,
      style: {
        height: 18,
        width: 18
      }
    }), /*#__PURE__*/React.createElement(_reactNative.View, {
      style: {
        width: 13
      }
    })) : null, /*#__PURE__*/React.createElement(_Text.SingleLineText, {
      paletteType: 'title',
      textType: 'medium',
      style: {
        color: getColor('bg')
      }
    }, name)),
    RightText: count,
    RightIcon: hasArrow ? /*#__PURE__*/React.createElement(_Image.Icon, {
      name: 'chevron_right',
      style: {
        height: 20,
        width: 20,
        tintColor: getColor('icon')
      }
    }) : undefined,
    onClicked: onClicked,
    containerStyle: {
      marginHorizontal: 16
    }
  });
}
//# sourceMappingURL=ContactList.item.js.map