...
exports.HSLAToHexACore = HSLAToHexACore;
exports.HSLAToRGBA = HSLAToRGBA;
exports.HSLAToRGBACore = HSLAToRGBACore;
exports.HSLToHex = HSLToHex;
exports.HSLToRGB = HSLToRGB;
exports.RGBAToHSLA = RGBAToHSLA;
exports.RGBAToHSLACore = RGBAToHSLACore;
exports.RGBAToHexA = RGBAToHexA;
exports.RGBAToHexACore = RGBAToHexACore;
exports.RGBToHSL = RGBToHSL;
exports.RGBToHex = RGBToHex;
exports.changeOpacity = changeOpacity;
exports.changeOpacityFromHSLA = changeOpacityFromHSLA;
exports.changeOpacityFromHexA = changeOpacityFromHexA;
exports.changeOpacityFromRGBA = changeOpacityFromRGBA;
exports.getOpacity = getOpacity;
exports.getOpacityFromHSLA = getOpacityFromHSLA;
exports.getOpacityFromHexA = getOpacityFromHexA;
exports.getOpacityFromRGBA = getOpacityFromRGBA;
exports.hexAToHSLA = hexAToHSLA;
exports.hexAToHSLACore = hexAToHSLACore;
exports.hexAToRGBA = hexAToRGBA;
exports.hexAToRGBACore = hexAToRGBACore;
exports.hexToHSL = hexToHSL;
exports.hexToRGB = hexToRGB;
var _colorConverter = require("./colorConverter.const");
/// https://css-tricks.com/converting-color-spaces-in-javascript/
/// https://codepen.io/jacobq_/pen/WPRNRQ
/// https://www.rapidtables.com/convert/color/index.html
/// https://www.myfixguide.com/color-converter/

function RGBAToHexACore(rgba) {
...
function RGBToHex(rgb, isUpper) {
  if (_colorConverter.ex_rgb.test(rgb) === false) {
...
function RGBAToHexA(rgba, isUpper) {
  if (_colorConverter.ex_rgba.test(rgba) === false) {
    throw new Error('Invalid input color');
  }
...
function hexAToRGBACore(hex, isPct) {
  let r = 0,
    g = 0,
    b = 0,
    a = 1;
...
function hexToRGB(h, isPct) {
  if (_colorConverter.ex_hex.test(h) === false) {
    throw new Error('Invalid input color');
  }
  const ret = hexAToRGBACore(h, isPct);
  return isPct === true ? `rgb(${ret.r}%, ${ret.g}%, ${ret.b}%)` : `rgb(${ret.r}, ${ret.g}, ${ret.b})`;
}
function hexAToRGBA(h, isPct) {
  if (_colorConverter.ex_hexa.test(h) === false) {
    throw new Error('Invalid input color');
  }
  const ret = hexAToRGBACore(h, isPct);
  return isPct === true ? `rgba(${ret.r}%, ${ret.g}%, ${ret.b}%, ${ret.a})` : `rgba(${ret.r}, ${ret.g}, ${ret.b}, ${ret.a})`;
}
function RGBAToHSLACore(rgba) {
  const rgbaString = rgba;
  const length = rgba.length;
  const rgbaNumber = length === 3 ? [0, 0, 0] : [0, 0, 0, 0];
  for (let index = 0; index < rgbaString.length; index++) {
...
function HSLAToRGBACore(hsla, isPct) {
  ...
  if (hString.indexOf('deg') > -1) {
    ...
    h = Math.round(+hString.substring(0, hString.length - 3) / (2 * Math.PI) * 360);
...
function HSLToRGB(hsl, isPct) {
  if (_colorConverter.ex_hsl.test(hsl) === false) {
    throw new Error('Invalid input color');
  }
...
function HSLAToRGBA(hsla, isPct) {
  if (_colorConverter.ex_hsla.test(hsla) === false) {
    throw new Error('Invalid input color');
  }
...
function hexAToHSLACore(hex) {
  const rgba = hexAToRGBACore(hex, false);
  if (rgba.a) {
    return RGBAToHSLACore([rgba.r.toString(), rgba.g.toString(), rgba.b.toString(), rgba.a.toString()]);
  } else {
    return RGBAToHSLACore([rgba.r.toString(), rgba.g.toString(), rgba.b.toString()]);
  }
}
function hexToHSL(hex) {
  if (_colorConverter.ex_hex.test(hex) === false) {
    throw new Error('Invalid input color');
  }
  const ret = hexAToHSLACore(hex);
  return `hsl(${ret.h}, ${ret.s}%, ${ret.l}%)`;
}
function hexAToHSLA(hex) {
  if (_colorConverter.ex_hexa.test(hex) === false) {
    throw new Error('Invalid input color');
  }
  const ret = hexAToHSLACore(hex);
  return `hsla(${ret.h}, ${ret.s}%, ${ret.l}%, ${ret.a})`;
}
function HSLAToHexACore(hsla) {
  const rgba = HSLAToRGBACore(hsla, false);
  if (rgba.a) {
    return RGBAToHexACore([rgba.r.toString(), rgba.g.toString(), rgba.b.toString(), rgba.a.toString()]);
  }
  return RGBAToHexACore([rgba.r.toString(), rgba.g.toString(), rgba.b.toString()]);
}
function HSLToHex(hsl, isUpper) {
  if (_colorConverter.ex_hsl.test(hsl) === false) {
    throw new Error('Invalid input color');
  }
  let sep = hsl.indexOf(',') > -1 ? ',' : ' ';
...
function HSLAToHexA(hsla, isUpper) {
  if (_colorConverter.ex_hsla.test(hsla) === false) {
    throw new Error('Invalid input color');
  }
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
...
function getOpacityFromHSLA(hsla) {
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
...
function getOpacity(color) {
  if (_colorConverter.ex_rgba.test(color) === true) {
    return getOpacityFromRGBA(color);
  } else if (_colorConverter.ex_hexa.test(color) === true) {
    return getOpacityFromHexA(color);
  } else if (_colorConverter.ex_hsla.test(color) === true) {
    return getOpacityFromHSLA(color);
  } else {
    return undefined;
  }
}
function changeOpacityFromRGBA(rgba, opacity) {
  let sep = rgba.indexOf(',') > -1 ? ',' : ' ';
  const rgbaString = rgba.substring(5).split(')')[0].split(sep);
  rgbaString[3] = opacity.toString();
  return `rgba(${rgbaString.join(', ')})`;
}
function changeOpacityFromHexA(hex, opacity) {
  let r = hex[1] + hex[2],
    g = hex[3] + hex[4],
    b = hex[5] + hex[6],
...
function changeOpacityFromHSLA(hsla, opacity) {
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
  const hslString = hsla.substring(5).split(')')[0].split(sep);
