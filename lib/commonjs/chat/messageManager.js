...
class MessageCacheManagerImpl {
  ...
  bindOnMessagesReceived(messages) {
    (0, _utils.asyncTask)(this.emitConversationUnreadCountChanged.bind(this));
    ...
    const callback = {
      ...
      onProgress: (localMsgId, progress) => {
        ...
        if (isExisted) {
          ...
          const p = {
            [_const.gMessageAttributeFileProgress]: progress
          };
          msg.attributes = {
            ...msg.attributes,
            ...p
      ...
      onError: (localMsgId, _error) => {
        ...
        if (isExisted) {
          ...
          msg.status = _reactNativeChatSdk.ChatMessageStatus.FAIL;
  ...
  createRecallMessageTip(msg) {
    const tip = _reactNativeChatSdk.ChatMessage.createCustomMessage(msg.conversationId, _const.gCustomMessageRecallEventType, msg.chatType, {
      params: {
  ...
  async recallMessage(msg) {
    ...
    const currentTimestamp = (0, _utils.getCurTs)();
    if (msg.localTime + this._recallTimeout < currentTimestamp) {
      this.emitRecallMessageChanged({
        isOk: false
      });
      this._client.sendError({
        error: new _error2.UIKitError({
          code: _error2.ErrorCode.msg_recall_error
        })
      });
      return;
    }
  ...
  async downloadAttachment(msg) {
    if (msg.body.type !== _reactNativeChatSdk.ChatMessageType.IMAGE && msg.body.type !== _reactNativeChatSdk.ChatMessageType.VIDEO && msg.body.type !== _reactNativeChatSdk.ChatMessageType.FILE && msg.body.type !== _reactNativeChatSdk.ChatMessageType.VOICE) {
      return;
    }
    const callback = {
      onSuccess: message => {
      ...
      onError: (localMsgId, _error) => {
        ...
        if (isExisted) {
          ...
          msg.body = {
            ...msg.body,
            fileStatus: _reactNativeChatSdk.ChatDownloadStatus.FAILED
          };
