...
exports.getChatServiceImpl = getChatServiceImpl;
var _reactNativeChatSdk = require("react-native-chat-sdk");
var _storage = require("../db/storage");
var _error = require("../error");
var _services = require("../services");
var _utils = require("../utils");
var _chat = require("./chat.listener");
var _const = require("./const");
var _messageManager = require("./messageManager");
var _requestList = require("./requestList");
var _types = require("./types.ui");
var _utils2 = require("./utils");
class ChatServiceImpl extends _chat.ChatServiceListenerImpl {
  constructor() {
    ...
    this._request = new _requestList.RequestListImpl(this);
    this._messageManager = new _messageManager.MessageCacheManagerImpl(this);
  }
  ...
  async init(params) {
    ...
    try {
      var _params$result;
      await this.client.init(new _reactNativeChatSdk.ChatOptions({
        ...options
      }));
      console.log('dev:chat:opt:', this.client.options);
      this._convStorage = new _storage.ConversationStorage({
        appKey: appKey
      });
      ...
      (_params$result2 = params.result) === null || _params$result2 === void 0 ? void 0 : _params$result2.call(params, {
        ...
        error: new _error.UIKitError({
          code: _error.ErrorCode.init_error,
  ...
  async _createUserDir() {
    try {
      const isExisted = await _services.Services.dcs.isExistedUserDir();
      if (isExisted !== true) {
        await _services.Services.dcs.createUserDir();
      }
    } catch (e) {
  ...
  async login(params) {
    ...
    try {
      ...
      _services.Services.dcs.init(`${this.client.options.appKey.replace('#', '-')}/${userId}`);
      ...
      result === null || result === void 0 ? void 0 : result({
        isOk: false,
        error: new _error.UIKitError({
          code: _error.ErrorCode.login_error,
          desc: this._fromChatError(error)
        })
      });
    }
  ...
  async logout(params) {
    try {
      ...
      (_params$result4 = params.result) === null || _params$result4 === void 0 ? void 0 : _params$result4.call(params, {
        isOk: false,
        error: new _error.UIKitError({
          code: _error.ErrorCode.logout_error,
          desc: this._fromChatError(error)
        })
      });
    }
    ...
    this.tryCatch({
      ...
      onError: error => {
        ...
        (_params$result9 = params.result) === null || _params$result9 === void 0 ? void 0 : _params$result9.call(params, {
          isOk: false,
          error: new _error.UIKitError({
            code: _error.ErrorCode.refresh_token_error,
            desc: this._fromChatError(error)
          })
        });
      }
  ...
  tryCatch(params) {
    ...
    promise.then(async value => {
      ...
      const _e = new _error.UIKitError({
        code: _error.ErrorCode.chat_uikit,
        tag: event,
  ...
  updateGroupParticipantOnRequestData(params) {
    ...
    if (list) {
      ...
      this.sendUIEvent(_types.UIListenerType.Group, 'onRequestReloadEvent', groupId);
    }
  }
  ...
  updateRequestData(params) {
    ...
    this.sendUIEvent(_types.UIListenerType.Contact, 'onRequestReloadEvent');
    this.sendUIEvent(_types.UIListenerType.Conversation, 'onRequestReloadEvent');
  }
  getRequestData(id) {
      ...
      if (this._basicDataRequestCallback) {
        ...
        Array.from(list.values()).forEach(v => {
          ...
          if (old === undefined || old.name === undefined || old.avatar === undefined) {
            needRequest.add({
              id: v.convId,
              type: v.convType === _reactNativeChatSdk.ChatConversationType.GroupChat ? 'group' : 'user',
              name: undefined,
              avatar: undefined
            });
  ...
  async getAllConversations(params) {
    ...
    try {
        ...
        if (map.size > 0) {
          ...
          silentList.forEach(v => {
            this._silentModeList.set(v.conversationId, {
              ...
              doNotDisturb: v.remindType === _reactNativeChatSdk.ChatPushRemindType.MENTION_ONLY || v.remindType === _reactNativeChatSdk.ChatPushRemindType.NONE
            });
          });
      ...
      onResult({
        isOk: false,
        error: new _error.UIKitError({
          code: _error.ErrorCode.get_all_conversations_error,
          desc: this._fromChatError(e)
        })
      });
    }
  ...
  async getConversation(params) {
    ...
    if (fromNative === true) {
      ...
      if (ret) {
        ...
        if (c2) {
          const conv = (0, _utils.mergeObjects)(c1, c2);
          this._convList.set(conv.convId, conv);
          return conv;
        } else {
          this._convList.set(c1.convId, c1);
          this.sendUIEvent(_types.UIListenerType.Conversation, 'onAddedEvent', c1);
        }
        return c1;
      }
    } else {
  ...
  async removeConversationAllMessages(params) {
    ...
    const conv = this._convList.get(params.convId);
    if (conv) {
      this.sendUIEvent(_types.UIListenerType.Conversation, 'onRequestReloadEvent', conv.convId);
    }
    _services.Services.dcs.deleteConversationDir(params.convId).then().catch(e => {
  ...
  async removeConversation(params) {
    ...
    if (conv) {
      ...
      this.sendUIEvent(_types.UIListenerType.Conversation, 'onDeletedEvent', conv);
  ...
  async clearAllConversations() {
    const ret = Array.from(this._convList.values()).map(async v => {
      ...
      this.sendUIEvent(_types.UIListenerType.Conversation, 'onDeletedEvent', v);
    });
    await Promise.all(ret);
    this._convList.clear();
  }
  async setConversationPin(params) {
    ...
    if (conv) {
      ...
      this.sendUIEvent(_types.UIListenerType.Conversation, 'onUpdatedEvent', conv);
    }
    return ret;
  }
  async setConversationSilentMode(params) {
    const ret = await this.tryCatchSync({
      promise: this.client.pushManager.setSilentModeForConversation({
        ...
        convType: params.convType,
        option: {
          remindType: params.doNotDisturb === true ? _reactNativeChatSdk.ChatPushRemindType.NONE : _reactNativeChatSdk.ChatPushRemindType.ALL,
          paramType: _reactNativeChatSdk.ChatSilentModeParamType.REMIND_TYPE
        }
      }),
  ...
  async getDoNotDisturb(convId, convType) {
    ...
    if (ret) {
      return (ret === null || ret === void 0 ? void 0 : ret.remindType) === _reactNativeChatSdk.ChatPushRemindType.MENTION_ONLY || (ret === null || ret === void 0 ? void 0 : ret.remindType) === _reactNativeChatSdk.ChatPushRemindType.NONE;
    }
    return false;
  }
  isContact(params) {
      ...
      this.tryCatch({
        ...
        onFinished: async value => {
          ...
          value.forEach(async v => {
            ...
            if (conv) {
              list.set(v.userId, (0, _utils.mergeObjects)(v, conv));
            } else {
              list.set(v.userId, {
                ...v
              });
  ...
  addNewContact(params) {
    this.tryCatch({
      ...
      onFinished: async () => {
        ...
        this.sendUIEvent(_types.UIListenerType.Contact, 'onAddedEvent', contact);
  ...
  removeContact(params) {
    this.tryCatch({
      ...
      onFinished: async () => {
        ...
        this.sendUIEvent(_types.UIListenerType.Contact, 'onDeletedEvent', contact);
    ...
    this.tryCatch({
      ...
      onFinished: () => {
        ...
        if (contact) {
          ...
          this.sendUIEvent(_types.UIListenerType.Contact, 'onUpdatedEvent', contact);
        }
    ...
    this.tryCatch({
      ...
      onFinished: value => {
        if (value) {
          ...
          if (localGroup) {
            this._groupList.set(group.groupId, (0, _utils.mergeObjects)(group, localGroup));
          } else {
  ...
  createGroup(params) {
    this.tryCatch({
      promise: this.client.groupManager.createGroup(new _reactNativeChatSdk.ChatGroupOptions({
        style: _reactNativeChatSdk.ChatGroupStyle.PrivateMemberCanInvite,
        maxCount: 1000,
      ...
      onFinished: async value => {
        ...
        this.sendUIEvent(_types.UIListenerType.Group, 'onAddedEvent', group);
    ...
    this.tryCatch({
      ...
      onFinished: async () => {
        ...
        if (group) {
          ...
          this.sendUIEvent(_types.UIListenerType.Group, 'onDeletedEvent', group);
        }
    ...
    this.tryCatch({
      ...
      onFinished: async () => {
        ...
        if (group) {
          ...
          this.sendUIEvent(_types.UIListenerType.Group, 'onUpdatedEvent', group);
        }
  ...
  setGroupMyRemark(params) {
    this.tryCatch({
      promise: this.client.groupManager.setMemberAttribute(params.groupId, params.memberId, {
        [_const.gGroupMemberMyRemark]: params.groupMyRemark
      }),
  ...
  getGroupMyRemark(params) {
    this.tryCatch({
      ...
      onFinished: result => {
        const group = this._groupList.get(params.groupId);
        if (group) {
          group.myRemark = result === null || result === void 0 ? void 0 : result[_const.gGroupMemberMyRemark];
        }
        params.onResult({
          isOk: true,
          value: result === null || result === void 0 ? void 0 : result[_const.gGroupMemberMyRemark]
        });
      }
    });
  }
    ...
    this.tryCatch({
      ...
      onFinished: async () => {
        ...
        if (groupMembers) {
          ...
          for (const member of params.members) {
            this.sendUIEvent(_types.UIListenerType.GroupParticipant, 'onAddedEvent', member);
          }
          params.onResult({
            isOk: true
          });
        ...
        for (const memberId of params.members) {
          ...
          if (groupMember) {
            ...
            if (member) {
              ...
              this.sendUIEvent(_types.UIListenerType.GroupParticipant, 'onDeletedEvent', member);
            }
          }
        }
        params.onResult({
      ...
      onFinished: async value => {
        if (value) {
          Array.from(value.values()).forEach(async v => {
            ...
            if (localUser) {
              this._userList.set(user.userId, (0, _utils.mergeObjects)(user, localUser));
            } else {
  ...
  getNewRequestList(params) {
    this.tryCatch({
      promise: this.client.chatManager.getMessagesWithMsgType(params.convId, params.convType, _reactNativeChatSdk.ChatMessageType.CUSTOM, params.direction, params.timestamp, params.pageSize),
  ...
  userInfoFromMessage(msg) {
    return (0, _utils2.userInfoFromMessage)(msg);
  }
  setUserInfoToMessage(params) {
    return (0, _utils2.setUserInfoToMessage)(params);
  }
      ...
      onFinished: () => {
        ...
        if (userId) {
          this.listeners.forEach(v => {
            ...
            (_v$onPresenceStatusCh = v.onPresenceStatusChanged) === null || _v$onPresenceStatusCh === void 0 ? void 0 : _v$onPresenceStatusCh.call(v, [new _reactNativeChatSdk.ChatPresence({
              publisher: userId,
              statusDescription: status,
              lastTime: (0, _utils.getCurTs)('s').toString(),
