...
class RequestListImpl {
  ...
  emitNewRequestListChanged() {
    for (const listener of this._userList.values()) {
      (0, _utils.timeoutTask)(0, () => listener.onNewRequestListChanged([...this._newRequestList]));
    }
  }
  bindOnContactInvited(userId) {
    const isExisted = this._newRequestList.findIndex(v => {
      var _v$msg;
      const t2 = ((_v$msg = v.msg) === null || _v$msg === void 0 ? void 0 : _v$msg.body.type) === _reactNativeChatSdk.ChatMessageType.CUSTOM;
      if (t2 === true) {
        var _v$msg2, _v$msg3;
        ...
        const t1 = ((_v$msg3 = v.msg) === null || _v$msg3 === void 0 ? void 0 : _v$msg3.attributes[_const.gNewRequestConversationUserId]) === userId;
    ...
    const newMsg = _reactNativeChatSdk.ChatMessage.createCustomMessage(_const.gNewRequestConversationId, _const.gNewRequestConversationMsgEventType);
    ...
    const item = (0, _utils2.getNewRequest)(newMsg);
    if (item) {
  ...
  bindOnConversationEvent(event, convId, _convType) {
    if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONTACT_ACCEPT) {
      for (const request of this._newRequestList) {
    ...
    } else if (event === _reactNativeChatSdk.ChatMultiDeviceEvent.CONTACT_DECLINE) {
      for (const request of this._newRequestList) {
        if (request.requestId === convId) {
          this.removeRequest(request);
          break;
  ...
  getRequestList(params) {
    ...
    this._client.getNewRequestList({
      ...
      convType: 0,
      pageSize: 200,
      direction: _reactNativeChatSdk.ChatSearchDirection.UP,
      onResult: result => {
        if (result.isOk) {
          if (result.value) {
            const list = result.value.map(v => {
              return (0, _utils2.getNewRequest)(v);
            }).filter(v => {
              return v !== undefined;
            });
