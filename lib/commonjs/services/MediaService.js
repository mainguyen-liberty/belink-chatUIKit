...
exports.MediaServiceImplement = void 0;
var _reactNativeCreateThumbnail = require("@easemob/react-native-create-thumbnail");
var _react = _interopRequireDefault(require("react"));
var _reactNative = require("react-native");
var _file = require("../utils/file");
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
function _extends() { _extends = Object.assign ? Object.assign.bind() : function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends.apply(this, arguments); }
class MediaServiceImplement {
  constructor(option) {
    this.option = option;
    this.rootDir = '';
  ...
  createRootDir(rootDirName, DocumentDir) {
    const _rootDir = _reactNative.Platform.select({
  ...
  async save(options) {
    const basePath = options.basePath ?? _reactNative.Platform.select({
      android: this.option.fsModule.Dirs.CacheDir,
      default: this.option.fsModule.Dirs.DocumentDir
    });
    let downloadPath = `${basePath}/${options.fileName}`;
    if (!(0, _file.getFileExtension)(options.fileName)) {
      const extensionFromUrl = (0, _file.getFileExtension)(options.fileUrl);
      if ((0, _file.getFileType)(extensionFromUrl).match(/image|video/)) {
    ...
    const fileType = (0, _file.getFileType)((0, _file.getFileExtension)(options.fileUrl));
    if (_reactNative.Platform.OS === 'ios' && (fileType === 'image' || fileType === 'video')) {
      ...
      await this.option.fsModule.FileSystem.cpExternal(downloadPath, (0, _file.generateFileName)(options.fileName, (0, _file.getFileExtension)(options.fileUrl)), dirType[fileType]);
    } else {
  ...
  async getVideoThumbnail(options) {
    try {
      ...
      const {
        path
      } = await (0, _reactNativeCreateThumbnail.createThumbnail)({
...
exports.MediaServiceImplement = MediaServiceImplement;
