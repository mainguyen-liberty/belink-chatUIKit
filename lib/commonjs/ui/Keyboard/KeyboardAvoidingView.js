...
class KeyboardAvoidingView extends React.Component {
  ...
  async _relativeKeyboardHeight(keyboardFrame) {
    ...
    if (_reactNative.Platform.OS === 'ios' && keyboardFrame.screenY === 0 && (await _reactNative.AccessibilityInfo.prefersCrossFadeTransitions())) {
      return 0;
    }
  ...
  _onKeyboardChange = event => {
    if (this._keyboardEvent !== undefined || this._keyboardEvent !== null) {
      this._keyboardEvent = _reactNative.Platform.OS === 'ios' ? event : {
        ...event,
        duration: 250,
        easing: 'linear'
      };
  ...
  componentDidMount() {
    if (_reactNative.Platform.OS === 'ios') {
      this._subscriptions = [_reactNative.Keyboard.addListener('keyboardWillChangeFrame', this._onKeyboardChange.bind(this))];
    } else {
      this._subscriptions = [_reactNative.Keyboard.addListener('keyboardDidHide', this._onKeyboardChange.bind(this)), _reactNative.Keyboard.addListener('keyboardDidShow', this._onKeyboardChange.bind(this))];
    }
  }
