...
function toCodePointText(text) {
  ...
  for (const key of gEmojiList) {
    ...
    tmp = tmp.replace(new RegExp(keyTmp, 'g'), _emoji.convert.fromCodePoint(key.substring(2)));
  }
  return tmp;
}
...
function fromCodePointText(text) {
  ...
  for (const key of gEmojiListUTF16) {
    tmp = tmp.replace(new RegExp(key, 'g'), 'U+' + _emoji.convert.toCodePoint(key).toUpperCase());
  }
  return tmp;
}
