...
import { ex_hex, ex_hexa, ex_hsl, ex_hsla, ex_rgb, ex_rgba } from './colorConverter.const';
export function RGBAToHexACore(rgba) {
...
export function RGBToHex(rgb, isUpper) {
...
export function RGBAToHexA(rgba, isUpper) {
...
export function hexAToRGBACore(hex, isPct) {
  let r = 0,
    g = 0,
    b = 0,
    a = 1;
...
export function RGBAToHSLACore(rgba) {
  const rgbaString = rgba;
  const length = rgba.length;
  const rgbaNumber = length === 3 ? [0, 0, 0] : [0, 0, 0, 0];
  for (let index = 0; index < rgbaString.length; index++) {
...
export function HSLAToRGBACore(hsla, isPct) {
...
export function hexAToHSLACore(hex) {
...
export function hexAToHSLA(hex) {
...
export function HSLAToHexACore(hsla) {
...
export function HSLToHex(hsl, isUpper) {
  if (ex_hsl.test(hsl) === false) {
    throw new Error('Invalid input color');
  }
  let sep = hsl.indexOf(',') > -1 ? ',' : ' ';
...
export function HSLAToHexA(hsla, isUpper) {
  if (ex_hsla.test(hsla) === false) {
    throw new Error('Invalid input color');
  }
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
...
export function getOpacityFromHSLA(hsla) {
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
  const hslString = hsla.substring(5).split(')')[0].split(sep);
  return +hslString[3];
}
export function getOpacityFromHexA(hex) {
  let a = 1;
  a = Number.parseInt('0x' + hex[7] + hex[8], 16);
  a = +(a / 255).toFixed(3);
  return a;
...
export function changeOpacityFromRGBA(rgba, opacity) {
  let sep = rgba.indexOf(',') > -1 ? ',' : ' ';
  const rgbaString = rgba.substring(5).split(')')[0].split(sep);
  rgbaString[3] = opacity.toString();
  return `rgba(${rgbaString.join(', ')})`;
}
export function changeOpacityFromHexA(hex, opacity) {
  let r = hex[1] + hex[2],
    g = hex[3] + hex[4],
    b = hex[5] + hex[6],
...
export function changeOpacityFromHSLA(hsla, opacity) {
  let sep = hsla.indexOf(',') > -1 ? ',' : ' ';
  const hslString = hsla.substring(5).split(')')[0].split(sep);
