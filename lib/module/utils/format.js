...
export function formatTs2(date, anchor, locale) {
...
export function formatTsForConvList(date, anchor, locale) {
  let d = typeof date === 'number' ? date : date.getTime();
  const n = anchor === undefined ? new Date().getTime() : typeof anchor === 'number' ? anchor : anchor.getTime();
  const oneDayBefore = n - 24 * 60 * 60 * 1000;
  if (d < oneDayBefore) {
...
export function formatTsForConvDetail(date, anchor, locale) {
  let d = typeof date === 'number' ? date : date.getTime();
  const n = anchor === undefined ? new Date().getTime() : typeof anchor === 'number' ? anchor : anchor.getTime();
  const oneDayBefore = n - 24 * 60 * 60 * 1000;
  if (d < oneDayBefore) {
