...
import { selectCamera, selectFile, selectOnePicture, selectOneShortVideo } from '../hooks/useSelectFile';
export function useMessageInput(props, ref) {
  ...
  const setInputValue = (text, op, face) => {
    if (op) {
      if (op === 'add_face') {
        ...
        valueRef.current = valueRef.current + emoji.convert.fromCodePoint(face.substring(2));
  ...
  } = useDelayExecTask(175, React.useCallback(() => {
